...
class LIDC2DICOMConverter:
  ...
  def convertForSubject(self, subjectID):
    ...
    tempSubjectDir = os.path.join(self.tempDir,s)
    ...
    for nCount,nodule in enumerate(scan.cluster_annotations()):
      ...
      for aCount,a in enumerate(nodule):
        ...
        converterCmd = [os.path.join(self.dcmqiRoot,'tid1500writer'), "--inputMetadata", jsonSRFile, "--inputImageLibraryDirectory", seriesDir, "--inputCompositeContextDirectory", tempSubjectDir, "--outputDICOM", dcmSRFile]
    ...
    self.cleanUpTempDir(tempSubjectDir)
